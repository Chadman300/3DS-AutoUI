...
static void netThreadMain(void* arg) {
    ...
    while (nt->running) {
        ...
        if (nt->live) {
            ...
            svcSleepThread(5 * 1000000LL); // brief gap between prioritized PID requests
        } else {
...
int main(int argc, char** argv) {
    ...
    while (aptMainLoop()) {
        ...
        if (!confirmRevert && settings.editMode && (keys & KEY_A)) settings.visible[selectedGauge] = !settings.visible[selectedGauge];
        if (keys & KEY_X) {
            static const unsigned int backgrounds[] = {0x0B1220, 0x101010, 0x18212B, 0x22141A};
            settings.theme = (settings.theme + 1) % 4;
            settings.background = backgrounds[settings.theme];
